import { useState, useEffect, useRef } from 'react'
import { WA_LINK } from '../../data/constants'

const ArrowRight = () => (
  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M5 12h14M12 5l7 7-7 7"/>
  </svg>
)

const WaIcon = () => (
  <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" style={{ flexShrink: 0 }}>
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
  </svg>
)

const links = [
  { href: '#servicios',  label: 'Servicios' },
  { href: '#dynojet',    label: 'Banco Dynojet' },
  { href: '#karting',    label: 'Karting' },
  { href: '#proceso',    label: 'Método ARP' },
  { href: '#galeria',    label: 'El taller' },
  { href: '#formulario', label: 'Contacto' },
]

export default function Navbar() {
  const [scrolled, setScrolled] = useState(false)
  const [open, setOpen] = useState(false)
  const firstLinkRef = useRef(null)

  useEffect(() => {
    const h = () => setScrolled(window.scrollY > 60)
    window.addEventListener('scroll', h, { passive: true })
    return () => window.removeEventListener('scroll', h)
  }, [])

  // Body scroll lock + first-link focus
  useEffect(() => {
    if (open) {
      document.body.style.overflow = 'hidden'
      const t = setTimeout(() => firstLinkRef.current?.focus(), 50)
      return () => clearTimeout(t)
    } else {
      document.body.style.overflow = ''
    }
    return () => { document.body.style.overflow = '' }
  }, [open])

  // Escape to close
  useEffect(() => {
    const onKey = (e) => { if (e.key === 'Escape') setOpen(false) }
    document.addEventListener('keydown', onKey)
    return () => document.removeEventListener('keydown', onKey)
  }, [])

  const close = () => setOpen(false)

  return (
    <>
      <nav className={`nav${scrolled ? ' scrolled' : ''}`}>
        <div className="nav-inner">
          <a href="#inicio" className="nav-brand" onClick={close}>
            <img
              src="/andres_racing_logo_sin_fondo.webp"
              alt="Andrés Racing Performance"
              className="nav-logo-img"
              fetchPriority="high"
              decoding="async"
            />
          </a>

          <button
            className={`nav-ham${open ? ' open' : ''}`}
            onClick={() => setOpen(o => !o)}
            aria-expanded={open}
            aria-controls="nav-mobile-menu"
            aria-label={open ? 'Cerrar menú' : 'Abrir menú'}
          >
            <span /><span /><span />
          </button>

          {/* Desktop links */}
          <div className="nav-links">
            {links.map(l => (
              <a key={l.href} href={l.href}>{l.label}</a>
            ))}
            <a className="nav-cta" href={WA_LINK} target="_blank" rel="noopener noreferrer">
              Consultar turno <ArrowRight />
            </a>
          </div>
        </div>
      </nav>

      {/* Mobile full-screen overlay */}
      <div
        id="nav-mobile-menu"
        className={`nav-mobile-overlay${open ? ' open' : ''}`}
        aria-hidden={!open}
        role="dialog"
        aria-label="Menú de navegación"
      >
        <div className="nav-mobile-backdrop" onClick={close} />
        <div className="nav-mobile-panel">
          <nav className="nav-mobile-links" aria-label="Navegación principal">
            {links.map((l, i) => (
              <a
                key={l.href}
                href={l.href}
                className="nav-mobile-link"
                style={{ '--i': i }}
                ref={i === 0 ? firstLinkRef : null}
                onClick={close}
              >
                <span className="nav-mobile-link-num">0{i + 1}</span>
                {l.label}
              </a>
            ))}
          </nav>
          <div className="nav-mobile-footer">
            <a
              className="nav-mobile-cta"
              href={WA_LINK}
              target="_blank"
              rel="noopener noreferrer"
              style={{ '--i': links.length }}
              onClick={close}
            >
              <WaIcon /> Consultar turno
            </a>
            <p className="nav-mobile-note">Respondemos el mismo día por WhatsApp</p>
          </div>
        </div>
      </div>
    </>
  )
}
